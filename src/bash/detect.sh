...
then
                        ...
                        then
                                Nthreads=1
                                bis_parallel=8
                                if [ $npar -gt 88 ]; then npar=5 && bis_parallel=16 && Nthreads=5 ;
                                        elif [ $npar -gt 64 -a $npar -le 88 ]; then npar=4 && bis_parallel=16 && Nthreads=4 ;
                                        elif [ $npar -gt 32 -a $npar -le 64 ]; then npar=3 && bis_parallel=16 && Nthreads=3 ;
                                        elif [ $npar -gt 16 -a $npar -le 32 ]; then npar=2 && bis_parallel=16 && Nthreads=2 ;
                                        elif [ $npar -gt 2 -a $npar -le 16 ]; then  npar=1 && sed -i "s/parallel_mode=.*/parallel_m
ode=false/g" config/pipeline.conf;
                                fi
