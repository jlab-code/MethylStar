...
if $run_pair_bismark; then
	...
	for fq in $(grep "_paired$first_pattern" $tmp_bismap/tmp.lst)
		do 
			...
			if $nucleotide; then
				echo "-- Nucleotide coverage is enabled. Running bismark for $label ..." 2>&1 | tee -a $tmp_clog/bismark-mapper.log
				...
				result=$($bismark_path/bismark -s 0 -u 0 -N 0 -L 20 --pbat --score_min L,-0.6,-0.6 --un \
							...
							--nucleotide_coverage --genome $genome_ref -1 \
							$tmp_fq/$file1 $tmp_fq/$file2 -2 $tmp_fq/$file3 $tmp_fq/$file4 -o $tmp_bismap/ 2>&1 \
							| tee -a $tmp_bismap/$label.log )
				# this command below is used for conventional WGBS libraries.
				#result=$($bismark_path/bismark -s 0 -u 0 -N 0 -L 20 --un --samtools_path $samtools_path --parallel \
				#			$bis_parallel -p $Nthreads --nucleotide_coverage --genome $genome_ref -1 \
				#			$tmp_fq/$file1 $tmp_fq/$file2 -2 $tmp_fq/$file3 $tmp_fq/$file4 -o $tmp_bismap/ 2>&1 \
				#			| tee -a $tmp_bismap/$label.log )
			else
				echo "-- Nucleotide coverage is disabled." 2>&1 | tee -a $tmp_clog/bismark-mapper.log
				echo "-- Running bismark for $file1 , $file2 and $file3 , $file4 ..." 2>&1 | tee -a $tmp_clog/bismark-mapper.log
				result=$($bismark_path/bismark -s 0 -u 0 -n 0 -l 20 --un --samtools_path $samtools_path --parallel $bis_parallel  -p $Nthreads --genome $genome_ref -1 $tmp_fq/$file1 $tmp_fq/$file2 -2 $tmp_fq/$file3 $tmp_fq/$file4 -o $tmp_bismap/ 2>&1 | tee -a $tmp_bismap/$label.log)
			fi
	...
	for fq in $(grep "_paired$first_pattern" $tmp_bismap/tmp.lst)
		do
			...
			if $nucleotide; then
				...
				echo "-- Nucleotide coverage is disabled." 2>&1 | tee -a $tmp_clog/bismark-mapper.log
				echo "-- Running bismark for $file1 and $file2 ..." 2>&1 | tee -a $tmp_clog/bismark-mapper.log
				result=$($bismark_path/bismark -N 1 -L 32 --un --samtools_path $samtools_path --parallel $bis_parallel -p $Nthreads --genome $genome_ref -1 $tmp_fq/$file1 -2 $tmp_fq/$file2 -o $tmp_bismap/ 2>&1 | tee -a $tmp_bismap/$label.log)
			fi
			echo $tmp_fq/$file1 >> $tmp_bismap/list-finished.lst;
			echo $tmp_fq/$file2 >> $tmp_bismap/list-finished.lst;
			runtime=$((($(date +%s)-$start)/60))
